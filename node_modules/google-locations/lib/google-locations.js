...
GoogleLocations.prototype.photo = function(options, cb) {
  if (!options.photoreference) return cb({error: 'photoreference string required'});
  options = _.defaults(options, {
    language: 'en'
  });

  var result = _generatePhotoUrl(this, options, 'place', null);
  ...
  https.get(request_query, function(res) {
      ...
      .on('end', function() {
          ...
          try {
            ...
            console.log(request_query.href);
...
function _generatePhotoUrl(context, query, type, method) {
  ...
  var request_query = url.parse(url.format({
    ...
    pathname: context.config.path + type + '/' + (method ? method + '/' : '') + 'photo',
    query: query
  }));
